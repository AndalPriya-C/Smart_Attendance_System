...
'nuttrenaivijayasankar@gmail.com'")
...
datetime.datetime.fromtimestamp(ts).strftime('%H:%M:%S')
 ...
 period = "Am" if int(current_time.split(':')[0]) < 12 else "Fn"
 attendance = [str(ID), '', bb, '', str(date), '', str(timeStamp), '', period]
 else:
 Id = 'Unknown'
 bb = str(Id)
...
def send_mail():
 ...
 email_password = "oevv abjo wrlb bugn"
 to_address = "mailid@gmail.com"
 msg = MIMEMultipart()
 msg['From'] = email_address
 msg['To'] = to_address

 msg['Subject'] = "Attendance Sheet"
 attachment_path = "Attendance/Attendance_" + date + ".csv"
...
"Attendance_" + date + ".csv")
 ...
 server.quit()
 mess.showinfo("Email Sent", "Attendance sheet sent successfully!")
 except Exception as e:
 mess.showerror("Error", f"An error occurred: {str(e)}")
def load_attendance_data(file_path):
 data = pd.read_csv(file_path) # Adjust based on your data storage format
 return data

def daily_attendance_trend(data):
 daily_trend = data.groupby('Date')['Id'].count()
 return daily_trend
def weekly_attendance_trend(data):
 data['Date'] = pd.to_datetime(data['Date'])
 weekly_trend = data.resample('W-Mon', on='Date')['Id'].count()
 return weekly_trend
def monthly_attendance_trend(data):
 data['Date'] = pd.to_datetime(data['Date'])
 monthly_trend = data.resample('M', on='Date')['Id'].count()
 return monthly_trend
def plot_attendance_trend(trend_data, title, xlabel, ylabel):
...
def show_attendance_analytics():
 data = load_attendance_data("D:/face final/Face Recognition Based 
Attendance Monitoring System/Attendance/Attendance_02-02-2024.csv") # 
Adjust based on your data file

 daily_trend = daily_attendance_trend(data)
 weekly_trend = weekly_attendance_trend(data)
 monthly_trend = monthly_attendance_trend(data)
 plot_attendance_trend(daily_trend, "Daily Attendance Trend", "Date", 
"Attendance Count")
 plot_attendance_trend(weekly_trend, "Weekly Attendance Trend", "Week", 
"Attendance Count")
 plot_attendance_trend(monthly_trend, "Monthly Attendance Trend", 
"Month", "Attendance Count")
data = load_attendance_data(r"D:/face final/Face Recognition Based Attendance 
Monitoring System/Attendance/Attendance_02-02-2024.csv")
def load_attendance_data(file_path="D:/face final/Face Recognition Based 
Attendance Monitoring System/Attendance/Attendance_02-02-2024.csv"):
 try:
 data = pd.read_csv("D:/face final/Face Recognition Based Attendance 
Monitoring System/Attendance/Attendance_02-02-2024.csv")
 return data
 ...
 return None
def load_latest_attendance():

 attendance_files = glob.glob("attendance/*.csv")
 latest_file = max(attendance_files, key=os.path.getctime)
 attendance_df = pd.read_csv(latest_file)
 return attendance_df
def send_messages():
 account_sid = 'AC224bffba73a38fde7a4fc7f60d3559b5'
 auth_token = '4105aba48c20860d9861d3ae0b7f2404'
 client = Client(account_sid, auth_token)
 message_body = 'Your child was absent from school today.'
 attendance_df = load_latest_attendance()
 phone_details_df = pd.read_csv("PhoneDetails.csv")
 present_students = attendance_df["Name"].tolist()
 all_students = phone_details_df["Name"].tolist()
 students_to_notify = [student for student in all_students if student not in 
present_students]
 for student in students_to_notify:
 parent_phone_number = phone_details_df.loc[phone_details_df['Name'] 
== student, 'PhoneNumber'].values[0]
 try: 
messageclient.messages.create(body=message_body,from_='+15737992128',to
=parent_phone_number)

 print(f"Message sent to parent of {student} with phone number 
{parent_phone_number}: {message.sid}")
 except Exception as e:
 mess.showerror("Error", f"An error occurred while sending messages: 
{str(e)}")
global key
key = ''
ts = time.time()
...
07':'July','08':'August','09':'September','10':'October','11':'November','12':'Decem
ber'}
window = tk.Tk()
...
message1=tk.Label(frame2,text="1)TakeImages>>>2)SaveProfile",bg="#c79cf
f",fg="black",width=39 ,height=1, activebackground = "#3ffc00" ,font=('comic', 
15, ' bold '))
...
send_message_button = tk.Button(frame1, text="Send Messages to Parents", 
command=send_messages, fg="black", bg="#ffa500", width=35, height=1, 
activebackground="white", font=('comic', 15, ' bold '))
send_message_button.place(x=30, y=500)
mailButton = tk.Button(frame1, text="Mail", command=send_mail, fg="black", 
bg="#ffa500", width=35, height=1,activebackground="white", font=('comic', 
15, ' bold '))
mailButton.place(x=30, y=450)
analytics_button = tk.Button(frame2, text="Show Attendance Analytics", 
command=show_attendance_analytics, fg="black", bg="#3ffc00", width=35, 
height=1, activebackground="white", font=('comic', 15, ' bold '))
analytics_button.place(x=30, y=500)
